...
void v4l2_set_format(int width, int height)
{
	...
	format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;		//图像采集
	//图像视频格式
	format.fmt.pix.width = width;	
	format.fmt.pix.height = height;
	format.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;//可选		一种存储色彩的压缩格式，可以用于传输
	printf("fd = %d\n", fd);
	int res = ioctl(fd, VIDIOC_S_FMT, &format);//设置帧格式
	
	//还可外加帧的速度  ioctl(fd,VIDIOC_S_PARM,&parm);
...
void v4l2_request_buffers()
{
	...
	reqbuffers.memory = V4L2_MEMORY_MMAP;//内存映射的方式,使用于连续的视频传输,速度快		另外还有直接读取的方式，速度较慢，用于静态图片的读取  用户指针使用较少
	int res = ioctl(fd, VIDIOC_REQBUFS, &reqbuffers);//分配内存
	if (res == -1)
	{
...
void v4l2_start_capture()
{
	...
	int res = ioctl(fd, VIDIOC_STREAMON, &type);//开启视频流
	if (res == -1)
	{
...
int v4l2_dequeue()
{
	struct v4l2_buffer buffer;
	buffer.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	int res = ioctl(fd, VIDIOC_DQBUF, &buffer);//出队
	if (res == -1)
	{
